...
function updateFilterDateDisplay() {
    ...
    var $filterDateTitle = $(".filter-title-bar>.filter-title").eq(0).children("span").eq(0); //获取入住时期
    if (startDate) {
        var text = startDate.substr(5) + "/" + endDate.substr(5); //从第5个开始抽取
...
$(document).ready(function () {
    ...
    var startDate = queryData["sd"]; //获取url中的开始时间
    var endDate = queryData["ed"];   //获取url中的结束时间
    $("#start-date").val(startDate);
    $("#end-date").val(endDate);

    updateFilterDateDisplay();

    var areaName = queryData["aname"]; //获取地区名字
...
$(document).ready(function () {
    ...
    $.get('/house/searchall/' + path, function (data) {
        if (data.code == '200') {
            var house_html = template('house_list', {hlist: data.houses});
