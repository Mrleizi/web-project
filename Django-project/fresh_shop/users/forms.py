...
class UserAddressForm(forms.Form):
    ...
    def clean_signer_postcode(self):
        postcode = self.cleaned_data['signer_postcode']
        if re.fullmatch(r'^[0-9]{6}$', postcode):
            return postcode
        else:
            raise forms.ValidationError('邮编格式有误')

    def clean_signer_mobile(self):
        mobile = self.cleaned_data['signer_mobile']
